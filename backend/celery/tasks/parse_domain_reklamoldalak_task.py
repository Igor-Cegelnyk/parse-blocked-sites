...
def run_parse_domain_reklamoldalak_task(self):
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            asyncio.ensure_future(loader_parse_domains(settings.advertising_api))
        else:
            loop.run_until_complete(loader_parse_domains(settings.advertising_api))
        return {
